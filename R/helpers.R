...
# =~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=#
nodePos <- function(
    nodes, networkGroup, edgeRad = 0.975, groupRad = 0.5,
...
# =~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=#
linkCol <- function(
    links, networkGroup, focus = NULL, colLink = "#876b40",
    shadowLink = "#f4f4f4") {
  ...
  if (is.null(focus)) {
    ...
    linkCol <- data.frame(from = nodes$subnetwork[match(
      links$from,
      nodes$category
    )], to = nodes$subnetwork[match(
      links$to,
      nodes$category
    )])
...
# =~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=#
boxGroup <- function(
    links, networkGroup, rad1 = 0.95, rad2 = 1,
  ...
  for (i in seq_len(nrow(networkGroup))) {
    ...
    if (type == "box") {
      graphics::polygon(rbind(a, b, a[1L, ]),
        col = colBox[i],
        ...
      )
    }
  ...
  for (i in seq_len(nrow(links))) {
    ...
    if (nodes$subnetwork[edgeFromID] != nodes$subnetwork[edgeToID]) {
      linkPath <- rbind(
        ...
        networkGroup[groupFromID, c("x", "y")], networkGroup[
          groupToID,
          c("x", "y")
      ...
      linkPath <- rbind(
        nodes[edgeFromID, c("x", "y")],
        networkGroup[groupFromID, c("x", "y")], nodes[
          edgeToID,
          c("x", "y")
        ]
      )
...
# =~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=~-~=#
metanetwork_legend <- function(
    labs, res = 300, textSize = 1,
    filename = NULL) {
  ...
  for (i in seq_len(nrow(net))) {
    graphics::lines(x = c(net$x1[i], net$x2[i]), y = c(
      1.25,
      1.25
    ), lwd = 2)
  }
  ...
  graphics::text(
    x = net$x1, y = 2, labels = net$network, cex = textSize,
    font = 2, adj = c(0, 0.5)
  )

  # Subnetworks
  graphics::rect(sub$x1, sub$y1, sub$x2, sub$y2,
    col = sub$col,
    border = sub$col
  )
  graphics::text(
    x = sub$x1 + xG, y = 0.5, labels = sub$subnetwork,
    cex = textSize * 0.9, font = 2, adj = c(0, 0.5)
  )

  # Categories
  graphics::text(
    x = cat$x + xG, y = -cat$y, labels = cat$labels,
...
plot0 <- function(
  ...
  if (length(args) > 0) {
    ...
    do.call(graphics::plot.default, args = as.list(c(
      coor,
      args, deft
    )))
  } else {
...
plotAreaColor <- function(color = "grey80", border = NA, ...) {
  ...
  ##
  if (length(args) > 0) {
    do.call(graphics::rect, args = as.list(c(coor,
      border = border,
      col = color, args
    )))
  } else {
